...
| `character-registry` | Any named character reusable across stories without re-authoring style objects | `characters/registry.json` + `Person.of(name)` per CONTRACTS §Person API (currently unimplemented — `people.js` has no `Person` class yet) | **M** |
...
| `giant-scale` | Kumbhakarṇa, Hiḍimba's true form, Vibhīṣaṇa's giants | Not new geometry — a scale multiplier + proportional camera/parallax adjustment so giants read correctly against human-scale sets | **S** |

### v2.3 — "Spectacle"

| Feature ID | Unlocks | Approach | Effort |
|---|---|---|---|
| `astra-fx` | Every astra duel (Brahmāstra, Āgneyāstra, Vāruṇāstra, Nāgāstra), Kṛṣṇa's Sudarśana | Particle/trail fx module keyed to astra name → element (fire/water/wind/serpent), triggered as storyboard `fx` beats already sketched in `stage.js`'s spec | **L** |
| `weather-morph-fx` | Rain-of-arrows, lightning, illusion/māyā transformations (Māyā Dānava, Rākṣasa shape-shifts) | Extend the same fx layer: density/opacity noise fields over `camLayer`; morph = crossfade between two style objects on one figure | **M** |
