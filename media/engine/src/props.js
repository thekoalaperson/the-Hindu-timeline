...
// one leg's hip/knee angles for a cycle position (0..1). Optional `stance`
// adds a constant idle-pose bias (see _stance) — locomotion passes nothing.
function _legAng(cyc, amp, stance) {
  const a = (cyc % 1) * TAU;
  const st = stance || 0;
  return {
    hip: Math.cos(a) * 0.26 * amp + (st ? st.hip : 0),  // fwd(+) at cyc 0, back(-) at cyc .5
    knee: 0.12 + Math.max(0, -Math.sin(a)) * 0.5 * amp + (st ? st.knee : 0), // flexes (tucks) through swing (.5..1)
  };
}
// idle standing stance. A real standing ungulate is never four parallel posts:
// the fore legs carry a slight fetlock/knee break, the hind legs tuck their
// hocks under the body. Returns {hip,knee} angle offsets, ZERO during walk/run
// so locomotion is untouched. role:+1 fore · -1 hind. k scales per animal
// (elephants stay near-columnar). Seed-jittered a degree or two → no twinning.
function _stance(gait, role, seed, n, k) {
  if (gait !== 'idle') return 0;
  const s = k == null ? 1 : k;
  const j = (hash1(seed * 4.7 + n * 2.9) - 0.5) * 0.05;    // ~±1.4°
  return role > 0
    ? { hip: (0.055 + j) * s, knee: (0.05 + j * 0.5) * s }       // fore: gentle break
    : { hip: (-0.12 + j) * s, knee: (0.175 + j * 0.5) * s };     // hind: hock angled under
}
...
function drawHorse(ctx, o) {
  ...
  _qLeg(ctx, foreF, upLen, loLen, _legAng(cyc(2), g.amp, _stance(gait, 1, seed, 2)), 1, legW.map(w => w * 0.92), _hx(coat, -0.12), coatD, _hx(hoof, -0.1), 'hoof');
  _qLeg(ctx, hindF, upLen, loLen, _legAng(cyc(3), g.amp, _stance(gait, -1, seed, 3)), -1, legW.map(w => w * 0.92), _hx(coat, -0.12), coatD, _hx(hoof, -0.1), 'hoof');
  ...
  // ── mane — a clean crest riding the neck and falling DOWN THE BACK; drawn
  //    BEFORE the head so it can never spill over the face ──
  const mSway = sfbm1(t * 1.1, seed + 2) * 5;
  ctx.fillStyle = mane;
  ctx.beginPath();
  ctx.moveTo(168, -316);
  ctx.quadraticCurveTo(210, -384, 286, -446);              // crest up to just behind the poll
  ctx.quadraticCurveTo(300, -438, 292, -420);              // small poll cap (tucks behind head)
  ctx.quadraticCurveTo(252, -402, 214, -360 + mSway * 0.5); // fall down the back of the neck
  ctx.quadraticCurveTo(196, -338, 182, -312 + mSway);
  ctx.quadraticCurveTo(174, -314, 168, -316);
  ctx.closePath(); ctx.fill();
  ctx.strokeStyle = _INK; ctx.lineWidth = 1.6; ctx.stroke();
  ctx.strokeStyle = _hx(mane, 0.16); ctx.lineWidth = 2.2; ctx.lineCap = 'round';
  for (let i = 0; i < 4; i++) {
    const u = (i + 0.5) / 4;
    const bx = lerp(196, 288, u), by = lerp(-320, -444, u);
    ctx.beginPath(); ctx.moveTo(bx, by); ctx.quadraticCurveTo(bx - 16, by + 24, bx - 26 + mSway * u, by + 42); ctx.stroke();
  }

  // ── head — constructed equine skull + round jowl + tapering muzzle wedge.
  //    (muzzle at +x/+y, poll & ears at -y, eye high on the skull) ──
  ctx.save();
  ctx.translate(300, -440); ctx.rotate(0.15);
  const earFlick = Math.sin(t * 2.1 + seed) * 0.12;
  // far ear (behind the poll)
  ctx.save(); ctx.translate(0, -16); ctx.rotate(0.36 + earFlick * 0.7);
  ctx.fillStyle = coatD;
  ctx.beginPath(); ctx.moveTo(0, 4); ctx.quadraticCurveTo(-9, -30, 8, -34); ctx.quadraticCurveTo(17, -14, 10, 4); ctx.closePath(); ctx.fill();
  ctx.strokeStyle = _INK; ctx.lineWidth = 1.5; ctx.stroke();
  ctx.restore();
  const head = [
    [-8, -20], [24, -19], [50, -9], [72, 9], [82, 29],     // poll → bridge → nose
    [77, 47], [60, 57], [40, 60], [16, 56],                // muzzle front → chin → jaw
    [-6, 44], [-17, 20], [-13, -3],                        // round jowl → back to poll
  ];
  ctx.beginPath(); smoothPath(ctx, head, true);
  const hhg = ctx.createLinearGradient(-10, -18, 66, 52);
  hhg.addColorStop(0, coatL); hhg.addColorStop(0.5, coat); hhg.addColorStop(1, coatD);
  ctx.fillStyle = hhg; ctx.fill();
  ctx.strokeStyle = _INK; ctx.lineWidth = 2.4; ctx.stroke();
  // modelling (clipped): round jowl shade · dark muzzle pad · bridge highlight
  ctx.save(); ctx.beginPath(); smoothPath(ctx, head, true); ctx.clip();
  ctx.fillStyle = rgba(coatD, 0.34); ctx.beginPath(); ctx.ellipse(4, 42, 24, 20, 0.2, 0, TAU); ctx.fill();
  ctx.fillStyle = rgba(_hx(coat, -0.3), 0.85); ctx.beginPath(); ctx.ellipse(72, 40, 16, 15, 0.2, 0, TAU); ctx.fill();
  ctx.fillStyle = 'rgba(255,246,214,0.16)'; ctx.beginPath(); ctx.ellipse(46, 8, 12, 30, 0.42, 0, TAU); ctx.fill();
  ctx.restore();
  // nostril (comma) + mouth line integrated into the muzzle silhouette
  ctx.fillStyle = '#160c06'; ctx.beginPath(); ctx.ellipse(74, 34, 5, 8, 0.2, 0, TAU); ctx.fill();
  ctx.strokeStyle = 'rgba(22,12,6,0.62)'; ctx.lineWidth = 1.6; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(80, 44); ctx.quadraticCurveTo(66, 54, 52, 52); ctx.stroke();
  // near ear — larger, alert, standing clear of the mane
  ctx.save(); ctx.translate(14, -18); ctx.rotate(-0.12 + earFlick);
  ctx.fillStyle = coat;
  ctx.beginPath(); ctx.moveTo(0, 6); ctx.quadraticCurveTo(-10, -42, 10, -40); ctx.quadraticCurveTo(20, -14, 12, 6); ctx.closePath(); ctx.fill();
  ctx.strokeStyle = _INK; ctx.lineWidth = 1.6; ctx.stroke();
  ctx.fillStyle = rgba(coatD, 0.55); ctx.beginPath(); ctx.moveTo(2, 2); ctx.quadraticCurveTo(-2, -26, 8, -30); ctx.quadraticCurveTo(11, -12, 8, 2); ctx.closePath(); ctx.fill();
  ctx.restore();
  // forelock — a short wisp between the ears, swept BACK over the poll (not the face)
  ctx.strokeStyle = mane; ctx.lineWidth = 3; ctx.lineCap = 'round';
  for (let i = -1; i <= 1; i++) { ctx.beginPath(); ctx.moveTo(8 + i * 5, -16); ctx.quadraticCurveTo(-2 + i * 4, -7, -12 + i * 3, -2); ctx.stroke(); }
  // eye — high on the skull, clear margin from the mane
  _animEye(ctx, 25, 5, 7.5, 0.42);
  ...
  _qLeg(ctx, hindN, upLen, loLen, _legAng(cyc(1), g.amp, _stance(gait, -1, seed, 1)), -1, legW, coat, coatD, hoof, 'hoof');
  _qLeg(ctx, foreN, upLen, loLen, _legAng(cyc(0), g.amp, _stance(gait, 1, seed, 0)), 1, legW, coat, coatD, hoof, 'hoof');
  ctx.restore();
}
...
function drawElephant(ctx, o) {
  ...
  _qLeg(ctx, foreF, upLen, loLen, _legAng(cyc(2), g.amp * 0.3, _stance(gait, 1, seed, 2, 0.5)), 1, legW.map(w => w * 0.94), _hx(skin, -0.1), skinD, _hx(nail, -0.2), 'pad');
  _qLeg(ctx, hindF, upLen, loLen, _legAng(cyc(3), g.amp * 0.3, _stance(gait, -1, seed, 3, 0.5)), -1, legW.map(w => w * 0.94), _hx(skin, -0.1), skinD, _hx(nail, -0.2), 'pad');
  ...
  ctx.save(); ctx.translate(0, -brB);
  // domed skull — Asian-elephant twin forehead + a defined brow, the lower face
  // flowing down into the trunk root so the head is one mass, not an ellipse
  const head = [
    [228, -462], [258, -502], [292, -516], [312, -506],    // left dome → central dip
    [334, -512], [356, -484], [372, -430],                 // right dome → brow
    [366, -356], [344, -326], [318, -318], [286, -322], [250, -346], [230, -390],
  ];
  ctx.beginPath(); smoothPath(ctx, head, true);
  const hg2 = ctx.createLinearGradient(230, -516, 330, -320);
  hg2.addColorStop(0, skinL); hg2.addColorStop(0.55, skin); hg2.addColorStop(1, skinD);
  ctx.fillStyle = hg2; ctx.fill();
  ctx.strokeStyle = _INK; ctx.lineWidth = 2.6; ctx.stroke();
  // forehead modelling: central furrow, twin-dome light, trunk-root shade
  ctx.save(); ctx.beginPath(); smoothPath(ctx, head, true); ctx.clip();
  ctx.strokeStyle = 'rgba(30,26,34,0.26)'; ctx.lineWidth = 2.4; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(313, -506); ctx.quadraticCurveTo(311, -440, 322, -360); ctx.stroke();
  ctx.fillStyle = 'rgba(255,250,244,0.10)'; ctx.beginPath(); ctx.ellipse(280, -486, 26, 34, -0.2, 0, TAU); ctx.fill();
  ctx.fillStyle = rgba(skinD, 0.24); ctx.beginPath(); ctx.ellipse(322, -350, 40, 46, 0, 0, TAU); ctx.fill();
  ctx.restore();
  // ear — a big veined leaf with a folded leading rim; slow deterministic fan
  const earFan = Math.sin(t * 0.6 + seed) * 0.05 + 0.05;
  ctx.save(); ctx.translate(258, -446);
  ctx.rotate(earFan);
  const eg = ctx.createLinearGradient(-40, -60, -90, 130);
  eg.addColorStop(0, _hx(skin, -0.04)); eg.addColorStop(1, _hx(skin, -0.2));
  ctx.fillStyle = eg;
  ctx.beginPath();
  ctx.moveTo(8, -66); ctx.quadraticCurveTo(-128, -78, -166, 26);       // top-back sweep
  ctx.quadraticCurveTo(-156, 132, -44, 150);                           // bottom lobe
  ctx.quadraticCurveTo(12, 104, 14, 4); ctx.closePath(); ctx.fill();   // front edge to root
  ctx.strokeStyle = _INK; ctx.lineWidth = 2.4; ctx.stroke();
  // folded leading rim (the ear's front edge rolls over)
  ctx.fillStyle = _hx(skin, 0.05);
  ctx.beginPath();
  ctx.moveTo(8, -62); ctx.quadraticCurveTo(-14, -22, -6, 64);
  ctx.quadraticCurveTo(2, 104, 14, 4); ctx.quadraticCurveTo(16, -34, 8, -62);
  ctx.closePath(); ctx.fill();
  ctx.strokeStyle = rgba(skinD, 0.5); ctx.lineWidth = 1.6; ctx.stroke();
  // veins radiating from the root toward the back edge
  ctx.strokeStyle = 'rgba(30,26,34,0.22)'; ctx.lineWidth = 1.5; ctx.lineCap = 'round';
  for (let i = 0; i < 5; i++) {
    const u = i / 4;
    ctx.beginPath(); ctx.moveTo(-4, 8);
    ctx.quadraticCurveTo(-70, -30 + u * 96, -134 - u * 8, -22 + u * 128);
    ctx.stroke();
  }
  if (o.ornament) { ctx.fillStyle = '#c11f2c'; ctx.beginPath(); ctx.arc(-78, 72, 16, 0, TAU); ctx.fill(); ctx.strokeStyle = GOLD; ctx.lineWidth = 2.5; ctx.stroke(); }
  ctx.restore();
  // eye — larger, with a heavy wrinkle crease above (elephant brow)
  _animEye(ctx, 322, -410, 10.5, 0.3);
  ctx.strokeStyle = 'rgba(30,26,34,0.4)'; ctx.lineWidth = 2.2; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(300, -430); ctx.quadraticCurveTo(322, -439, 348, -427); ctx.stroke();
  ctx.strokeStyle = 'rgba(30,26,34,0.24)'; ctx.lineWidth = 1.8;
  ctx.beginPath(); ctx.moveTo(304, -422); ctx.quadraticCurveTo(323, -430, 344, -420); ctx.stroke();
  // tusk
  ctx.fillStyle = nail;
  ...
  // near legs
  _qLeg(ctx, hindN, upLen, loLen, _legAng(cyc(1), g.amp * 0.3, _stance(gait, -1, seed, 1, 0.5)), -1, legW, skin, skinD, nail, 'pad');
  _qLeg(ctx, foreN, upLen, loLen, _legAng(cyc(0), g.amp * 0.3, _stance(gait, 1, seed, 0, 0.5)), 1, legW, skin, skinD, nail, 'pad');
  ctx.restore();
}
...
function drawDeer(ctx, o) {
  ...
  _qLeg(ctx, foreF, upLen, loLen, _legAng(cyc(2), g.amp, _stance(gait, 1, seed, 2)), 1, legW.map(w => w * 0.9), _hx(coat, -0.12), coatD, _hx(hoof, -0.1));
  _qLeg(ctx, hindF, upLen, loLen, _legAng(cyc(3), g.amp, _stance(gait, -1, seed, 3)), -1, legW.map(w => w * 0.9), _hx(coat, -0.12), coatD, _hx(hoof, -0.1));
  ...
  _qLeg(ctx, hindN, upLen, loLen, _legAng(cyc(1), g.amp, _stance(gait, -1, seed, 1)), -1, legW, coat, coatD, hoof);
  _qLeg(ctx, foreN, upLen, loLen, _legAng(cyc(0), g.amp, _stance(gait, 1, seed, 0)), 1, legW, coat, coatD, hoof);
  ctx.restore();
}
...
function drawBird(ctx, o) {
  ...
  if (gait === 'run') {
    ...
    // folded wing — ONE teardrop covert mass hugging the body and tapering over
    // the tail; soft feather rows (not tiling); 2-3 primary TIPS peeking at the
    // rear, their points converging just above the tail. Never blades crossing
    // the silhouette.
    const set = sfbm1(t * 0.5, seed + 8) * 1.5;             // tiny settle motion
    const mass = [
      [46, -8], [30, -20], [0, -23], [-30, -17], [-54, -3], // shoulder → dorsal edge → rear
      [-70, 7 + set],                                        // rear point, over the tail
      [-50, 16], [-14, 20], [22, 18], [44, 9],               // ventral edge → front
    ];
    ctx.beginPath(); smoothPath(ctx, mass, true);
    const wg = ctx.createLinearGradient(30, -20, -58, 16);
    wg.addColorStop(0, _hx(wingC, 0.08)); wg.addColorStop(1, _hx(wingC, -0.16));
    ctx.fillStyle = wg; ctx.fill();
    ctx.strokeStyle = _INK; ctx.lineWidth = 1.8; ctx.stroke();
    // feather rows — three soft scalloped arcs following the fold (front → rear)
    ctx.strokeStyle = rgba(bodyD, 0.42); ctx.lineWidth = 1.2; ctx.lineCap = 'round';
    for (let r = 0; r < 3; r++) {
      const yy = -8 + r * 8;
      ctx.beginPath(); ctx.moveTo(34, yy);
      ctx.quadraticCurveTo(-8, yy + 6, -42, yy + 9);
      ctx.quadraticCurveTo(-58, yy + 10, -66, yy + 8); ctx.stroke();
    }
    // 2-3 primary tips: narrow rounded feathers whose points converge over the tail
    for (let i = 0; i < 3; i++) {
      const bx = -30 - i * 6, by = i * 5;                   // bases spread inside the mass
      const tx = -76 - i * 4, ty = 11 + i * 1.4 + set * 0.5; // tips clustered (converge)
      const w = 3.8 - i * 0.4;
      ctx.fillStyle = _hx(wingC, -0.06 - i * 0.05);
      ctx.beginPath();
      ctx.moveTo(bx, by - w);
      ctx.quadraticCurveTo((bx + tx) / 2, (by + ty) / 2 - w, tx, ty);          // upper edge → rounded tip
      ctx.quadraticCurveTo((bx + tx) / 2, (by + ty) / 2 + w * 0.7, bx, by + w); // lower edge back
      ctx.closePath(); ctx.fill();
      ctx.strokeStyle = rgba(bodyD, 0.5); ctx.lineWidth = 0.9; ctx.stroke();    // subtle darker edge
    }
    // lacy dorsal aigrette — wispy breeding plumes that drape back and DROOP
    // over the tail (egret filoplumes), never straight horizontal blades
    const pl = sfbm1(t * 0.8, seed + 3) * 5;
    ctx.strokeStyle = rgba(body, 0.72); ctx.lineWidth = 1.4; ctx.lineCap = 'round';
    for (let i = 0; i < 3; i++) {
      ctx.beginPath(); ctx.moveTo(-6 + i * 3, -17 + i * 2);
      ctx.quadraticCurveTo(-46, -6 + pl * 0.4, -66 - i * 4, 15 + i * 3 + pl);
      ctx.stroke();
    }
  }
  ctx.restore();
